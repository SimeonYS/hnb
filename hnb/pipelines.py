...
class HnbPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('hnb.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `hnb`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM hnb WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO hnb (date, title, link, content)"
